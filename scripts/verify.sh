...
#
# A certificate must certify the file sitting next to it. This script is the
# only thing that proves that, so run it before every push. It needs curl and
# jq and nothing else, and it costs nothing: octra_compileAml is a free read.
#
...
for src in contracts/*.aml; do
  ...
  if ! echo "$out" | jq -e '.result' >/dev/null 2>&1; then
    echo "SKIP  $name: node did not answer (rate limit?), retry later"
    continue
  fi
...
if [ "$fail" -ne 0 ]; then
  echo
  echo "Something does not tally. Do not push."
  exit 1
fi
echo
